...
if(isset($_POST['submit'])){
	foreach ($_FILES['myImages']['tmp_name'] as $key=>$image) {
		...
		if($con->query("insert into nonveg(id,itemName,cost,imageName) values(NULL,'$itemName',$cost,'$imageName')") === TRUE){
	echo "Inserted Successfully";
	header("Location:http://localhost/TastyTouch/uploadNonVeg.php");
}
else{
	echo "Insert failed";
}
